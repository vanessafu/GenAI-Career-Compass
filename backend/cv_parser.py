import fitz
import logging
from backend.config import get_async_openai_client, OPENAI_MODEL, OPENAI_TEMPERATURE
from backend.cv_schema import CVData

logger = logging.getLogger("CareerCompass.Parser")
...
async def parse_cv_to_pydantic(raw_text: str) -> CVData:
    """Send the text to the LLM afnd enforce the Pydantic schema as output."""
    ...
    try:
        ...
        logger.error(f"LLM communication error: {exc}")
        raise RuntimeError("Error while processing with the LLM.")